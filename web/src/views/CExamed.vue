...
export default {
  ...
  methods: {
    countDateClick(name) {
      ...
      this.$.posts("/cexamed/find", { begin, end, teamed: name })
        .then(res => {
          this.ajaxs = res;
    ...
    selectChange(row) {
      ...
      // 查询标的对应测算税种列表
      this.$.posts("/cexamed/tax", { data_id: row.id })
        .then(res => {
          this.counts = res;
    ...
    toPass() {
      ...
      if (row === null) {
        this.$Message.error("没有选择审批标的！");
        return;
      }
      this.$.posts("/cexamed/pass", { uid: row.uid })
        .then(res => {
          this.current = null;

          xcon.arrsEdit(this.ajaxs, "uid", row.uid, res);
          this.$Message.success("案件审批通过！");
        })
        .catch(error => {
          this.$Message.error(error);
        });
  ...
  computed: {
    ...
    teamType() {
      return this.current && this.current.teamed_type <= 1
        ? "单独审批"
        : "集体审议";
    }
  },
  created() {
    this.$.gets("/cexamed/index")
      .then(res => {
        this.ajaxs = res;
        this.tableLoading = false;
      })
